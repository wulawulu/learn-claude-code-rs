...
use crate::{
    ToolSpec,
    team::{MessageType, SharedMessageBus},
    tool::Tool,
};
...
pub fn send_message_tool(bus: SharedMessageBus, sender_name: impl Into<String>) -> Box<dyn Tool> {
...
impl Tool for SendMessageTool {
    async fn invoke(&mut self, input: &Value) -> Result<String> {
        ...
        self.bus.send(&self.sender_name, to, content, message_type)
    }
